...
impl HeaderWithAncestors {
	...
	pub fn check_response(
		...
		let expected_hash = match (self.block_hash, start) {
			...
			_ => {
				warn!(target: "on_demand", "HeaderWithAncestors request by block number instead of hash");
...
impl HeaderByHash {
	...
	pub fn check_response(
		...
		let expected_hash = match (self.0, start) {
			...
			(_, &net_request::HashOrNumber::Hash(h2)) => h2,
			_ => {
				warn!(target: "on_demand", "HeaderByHash request by block number instead of hash");
				return Err(Error::HeaderByNumber)
			}
		};
